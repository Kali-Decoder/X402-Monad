    ...
    <Card className="bg-zinc-900/50 border-zinc-800 overflow-hidden hover:border-zinc-700 transition-all duration-200">
      <div className="p-6">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-start gap-3 flex-1">
            {agent.image ? (
              ...
              <div className="w-12 h-12 rounded-lg bg-gradient-to-br from-zinc-800 to-zinc-900 flex items-center justify-center border border-zinc-800">
        ...
        {agent.tasks.length > 0 && (
          <div className="mb-4">
            <p className="text-xs text-zinc-500 mb-2">Supported Tasks</p>
            <div className="flex flex-wrap gap-1.5">
              {agent.tasks.slice(0, 3).map((task: string, index: number) => (
                <span
                  key={index}
                  className="px-2 py-1 rounded bg-zinc-800/50 text-xs text-zinc-400 border border-zinc-800"
                >
                  {task}
                </span>
              ))}
              {agent.tasks.length > 3 && (
                <span className="px-2 py-1 rounded bg-zinc-800/50 text-xs text-zinc-400 border border-zinc-800">
